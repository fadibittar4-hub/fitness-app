...
src/
  app/
    ...
    features/
      ...
      trainee/
        shell/                    — top bar with logo + profile avatar
        pages/
          ...
          profile/                — confirmed bookings only, logout
      trainer/
      ...
      sessions/
        models/
          session.models.ts       — Session, CreateSessionRequest, UpdateSessionRequest
        services/
...
|---|---|---|
| Trainee | Fixed bottom nav (2 tabs) | Inline topbar nav, content max 72rem |
...
5. On `status === 'paid'` response the booking is confirmed — no separate booking creation step
...
| `amount` | `session.price` (fallback `49.99` if `0`) | Price shown and charged |
